...
Shader "Shader Forge/Diffuse Spec" {
    Properties {
        ...
        _BackPower ("Back Power", Range(0, 1)) = 0
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
////// Emissive:
                ...
                float node_2162 = 0.0;
                float3 node_5085 = (((node_544*(_BackPower + ( (node_7782 - node_2162) * (1.0 - _BackPower) ) / (1.0 - node_2162)))+(node_7782*pow(max(0,dot(normalDirection,halfDirection)),exp2(lerp(1,11,_Gloss)))*_SpecTex_var.r))*_LightColor0.rgb*attenuation); // Attenuate and Color
...
////// Lighting:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float3 node_544 = (_Diffuse_var.rgb*_Color.rgb); // Diffuse Color
                float4 _SpecTex_var = tex2D(_SpecTex,TRANSFORM_TEX(i.uv0, _SpecTex));
                o.Emission = ((node_544*UNITY_LIGHTMODEL_AMBIENT.rgb)+(_SpecTex_var.g*_RimColor.rgb*_RimPower));
