...
Shader "Custom/Plane Shadow" 
{
	Properties{
		...
		_ShadowPlane("Shadow Plane", Vector) = (0,1,0,0.02)//阴影平面
		_ShadowColor("Shadow Color",Color) = (0.2, 0.2, 0.2, 1)//阴影颜色
		_ShadowPara("Shadow Param",Vector) = (-0.006,60,3,0.02)//阴影过渡
		_lightDirection("LightDir",Vector) = (-2.33,-2.7,3.27,2.26)//模拟光照方向
	}
	...
	SubShader{
		...
		pass {
			...
			ZWrite Off						//关闭深度写入
			Fog { Mode Off }				//关闭雾效
			Blend SrcAlpha OneMinusSrcAlpha//半透明混合方式
			...
			{
				  v2f o;
				  fixed3 vt = mul(unity_ObjectToWorld , v.vertex).xyz;
				  fixed3 tmpvar_3 = (vt - (((dot(_ShadowPlane.xyz, vt) - _ShadowPlane.w) / dot(_ShadowPlane.xyz, _lightDirection)) * _lightDirection));
				  fixed4 tmpvar_4 = fixed4(tmpvar_3, 1.0);
